...
export default {
  ...
  methods: {
    ...
    handleRemove(file) {
      ...
      //3 调用数组的 splice 方法，把图片的信息对象，从pics 数组中移除
    ...
    add() {  
      this.$refs.addFormRef.validate( async valid => {
        ...
        //执行添加的业务逻辑
        //与商品添加按钮冲突，用深拷贝 lodash   cloneDeep(obj)
        const form = _.cloneDeep(this.addForm);
        form.goods_cat = form.goods_cat.join(',');
        //处理动态参数
        this.manTableDate.forEach(item =>{
            const newInfo = {
